...
// Extend HTMLVideoElement to include WebKit PiP properties
...
export function VideoPlayer({ channel, onClose }: VideoPlayerProps) {
  const videoRef = useRef<HTMLVideoElementWithPiP>(null);
  ...
  const togglePip = useCallback(async () => {
    ...
    try {
      ...
      if (isIOS()) {
        // Check if PiP is supported on this iOS device
        if (video.webkitSupportsPresentationMode?.('picture-in-picture')) {
          ...
          console.log('PiP not supported on this iOS device');
        }
        return;
      }
